...
const App: React.FC = () => {
  ...
  // CARS Specific Authentication State
  const [isCarsAuthenticated, setIsCarsAuthenticated] = useState(() => {
      return localStorage.getItem('cars_auth_token') === 'demo-session-active';
  });

  const handleCarsLogin = () => {
      setIsCarsAuthenticated(true);
      localStorage.setItem('cars_auth_token', 'demo-session-active');
  };

  const handleCarsLogout = () => {
      setIsCarsAuthenticated(false);
      localStorage.removeItem('cars_auth_token');
  };
  ...
  const [users, setUsers] = useState<User[]>([
      { id: 1, name: 'System Admin', email: 'pita.domingos@zd044.onmicrosoft.com', role: UserRole.SYSTEM_ADMIN, status: 'Active', company: 'Vulcan Mining', jobTitle: 'IT Manager', siteId: 's1' }
  ]);
  ...
  const [feedbackConfig, setFeedbackConfig] = useState<{mode: string, expiry: string | null}>({ mode: 'always', expiry: null });
  ...
  const handleSubmitFeedback = (type: FeedbackType, message: string) => {
      const newFeedback: Feedback = {
          id: uuidv4(), 
          userId: 'current-user', 
          userName: userRole === UserRole.USER ? 'Safe Worker 1' : 'Admin User', 
          type, 
          message, 
          status: 'New', 
          isActionable: type === 'Bug', 
  ...
  const isFeedbackSystemActive = useMemo(() => {
      ...
      if (feedbackConfig.expiry) return new Date() < new Date(feedbackConfig.expiry);
      return false;
  }, [feedbackConfig]);

  const addNotification = (notif: SystemNotification) => setNotifications(prev => [notif, ...prev]);

  const handleBookingsUpdate = (newBookings: Booking[]) => setBookings(prev => [...prev, ...newBookings]);
  ...
  const handleApproveAutoBooking = (bookingId: string) => {
      ...
      addNotification({ id: uuidv4(), type: 'success', title: 'Approved', message: 'Booking confirmed.', timestamp: new Date(), isRead: false });
  };

  const handleRejectAutoBooking = (bookingId: string) => setBookings(prev => prev.filter(b => b.id !== bookingId));
  ...
  const canViewAlcoholDashboard = (): boolean => {
      ...
      return allowedTitles.some(t => safeTitle.includes(t));
  };
            ...
            <Route path="*" element={
              <CarsProtectedRoute isCarsAuthenticated={isCarsAuthenticated}>
                <Layout 
                  ...
                  companies={companies}
                  onLogout={handleCarsLogout}
                >
                  <Routes>
                    <Route path="/dashboard" element={<Dashboard bookings={bookings} requirements={requirements} sessions={sessions} userRole={userRole} onApproveAutoBooking={handleApproveAutoBooking} onRejectAutoBooking={handleRejectAutoBooking} racDefinitions={racDefinitions} currentSiteId={currentSiteId} />} />
                    <Route path="/enterprise-dashboard" element={[UserRole.SYSTEM_ADMIN, UserRole.ENTERPRISE_ADMIN].includes(userRole) ? <EnterpriseDashboard sites={sites} bookings={bookings} requirements={requirements} userRole={userRole} /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/site-governance" element={[UserRole.SYSTEM_ADMIN, UserRole.ENTERPRISE_ADMIN, UserRole.SITE_ADMIN].includes(userRole) ? <SiteGovernancePage sites={sites} setSites={setSites} racDefinitions={racDefinitions} bookings={bookings} requirements={requirements} updateRequirements={handleUpdateRequirement} /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/database" element={userRole !== UserRole.USER && userRole !== UserRole.RAC_TRAINER ? <DatabasePage bookings={bookings} requirements={requirements} updateRequirements={handleUpdateRequirement} sessions={sessions} onUpdateEmployee={handleUpdateEmployee} onDeleteEmployee={handleDeleteEmployee} racDefinitions={racDefinitions} importBookings={handleImportBookings} addNotification={addNotification} currentSiteId={currentSiteId} /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/reports" element={<ReportsPage bookings={bookings} sessions={sessions} currentSiteId={currentSiteId} />} />
                    <Route path="/booking" element={userRole !== UserRole.RAC_TRAINER && userRole !== UserRole.ENTERPRISE_ADMIN && userRole !== UserRole.SITE_ADMIN ? <BookingForm addBookings={handleBookingsUpdate} sessions={sessions} userRole={userRole} existingBookings={bookings} addNotification={addNotification} currentEmployeeId={currentEmployeeId} requirements={requirements} racDefinitions={racDefinitions} /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/trainer-input" element={[UserRole.SYSTEM_ADMIN, UserRole.RAC_TRAINER].includes(userRole) ? <TrainerInputPage bookings={bookings} updateBookings={updateBookingsStatus} sessions={sessions} userRole={userRole} currentUserName="Instructor" racDefinitions={racDefinitions} /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/results" element={<ResultsPage bookings={bookings} updateBookingStatus={(id, status) => setBookings(p => p.map(b => b.id === id ? { ...b, status } : b))} importBookings={handleImportBookings} userRole={userRole} sessions={sessions} currentEmployeeId={currentEmployeeId} racDefinitions={racDefinitions} addNotification={addNotification} currentSiteId={currentSiteId} />} />
                    <Route path="/users" element={userRole === UserRole.SYSTEM_ADMIN ? <UserManagement users={users} setUsers={setUsers} addNotification={addNotification} sites={sites} currentSiteId={currentSiteId} /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/schedule" element={[UserRole.SYSTEM_ADMIN, UserRole.SITE_ADMIN].includes(userRole) ? <ScheduleTraining sessions={sessions} setSessions={setSessions} rooms={rooms} trainers={trainers} racDefinitions={racDefinitions} addNotification={addNotification} currentSiteId={currentSiteId} /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/settings" element={[UserRole.SYSTEM_ADMIN, UserRole.ENTERPRISE_ADMIN, UserRole.SITE_ADMIN].includes(userRole) ? <SettingsPage racDefinitions={racDefinitions} onUpdateRacs={handleUpdateRacDefinitions} rooms={rooms} onUpdateRooms={setRooms} trainers={trainers} onUpdateTrainers={setTrainers} sites={sites} onUpdateSites={setSites} companies={companies} onUpdateCompanies={setCompanies} userRole={userRole} users={users} onUpdateUsers={setUsers} feedbackConfig={feedbackConfig} onUpdateFeedbackConfig={handleUpdateFeedbackConfig} onSyncDatabases={handleMiddlewareSync} addNotification={addNotification} /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/request-cards" element={userRole !== UserRole.ENTERPRISE_ADMIN && userRole !== UserRole.RAC_TRAINER ? <RequestCardsPage bookings={bookings} requirements={requirements} racDefinitions={racDefinitions} sessions={sessions} userRole={userRole} currentEmployeeId={currentEmployeeId} currentSiteId={currentSiteId} /> : <Navigate to="/dashboard" replace />} />
                    ...
                    <Route path="/admin-manual" element={userRole === UserRole.SYSTEM_ADMIN ? <AdminManualPage /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/tech-docs" element={userRole === UserRole.SYSTEM_ADMIN ? <TechnicalDocs /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/logs" element={[UserRole.SYSTEM_ADMIN, UserRole.ENTERPRISE_ADMIN].includes(userRole) ? <LogsPage /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/source-control" element={userRole === UserRole.SYSTEM_ADMIN ? <SourceControlPage /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/feedback-admin" element={[UserRole.SYSTEM_ADMIN, UserRole.ENTERPRISE_ADMIN].includes(userRole) ? <FeedbackAdminPage feedbackList={feedbackList} onUpdateFeedback={handleUpdateFeedback} onDeleteFeedback={handleDeleteFeedback} /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/messages" element={userRole === UserRole.SYSTEM_ADMIN ? <MessageLogPage /> : <Navigate to="/dashboard" replace />} />
                    <Route path="/alcohol-control" element={canViewAlcoholDashboard() ? <AlcoholIntegration addNotification={addNotification} /> : <Navigate to="/dashboard" replace />} />
                  ...
                  {isFeedbackSystemActive && (
                      <>
                        <button onClick={() => setIsFeedbackModalOpen(true)} className="fixed bottom-6 right-24 z-40 bg-white dark:bg-slate-800 text-slate-500 hover:text-indigo-600 p-3 rounded-full shadow-lg border border-slate-200 no-print transition-all hover:scale-110" title="Feedback"><MessageSquarePlus size={24} /></button>
                        <FeedbackModal isOpen={isFeedbackModalOpen} onClose={() => setIsFeedbackModalOpen(false)} onSubmit={handleSubmitFeedback} />
                      </>
                  )}
                </Layout>
              </CarsProtectedRoute>
